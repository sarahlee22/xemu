...
void audio_set_stereo_parameters ( int vol, int sep )
{
	...
	stereo_separation_other = ((0x100 - sep) * vol) / 100;
}
...
static void audio_callback ( void *userdata, Uint8 *stereo_out_stream, int len )
{
	...
	for (int i = 0; i < len; i++) {
		...
		int orig_left  = streams[0][i] + streams[1][i] + streams[4][i] + streams[6][i];
		int orig_right = streams[2][i] + streams[3][i] + streams[5][i] + streams[6][i];
		// channel stereo separation (including inversion) + volume handling
		int left  = ((orig_left  * stereo_separation_orig) / 256) + ((orig_right * stereo_separation_other) / 256);
		int right = ((orig_right * stereo_separation_orig) / 256) + ((orig_left  * stereo_separation_other) / 256);
		//DEBUGPRINT("left=%d right=%d ", left, right);
